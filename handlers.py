...
async def forwarding_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"`{address}`\n"
        f"_(tap to copy)_\n\n"
        f"Forward your job emails here and I'll track them automatically — "
        f"no access to your inbox needed.\n\n"
        f"**Set it up once in Gmail:**\n"
        f"1. Settings → *Forwarding and POP/IMAP* → **Add a forwarding address**\n"
        f"2. Paste the address above. Gmail emails it a confirmation code — "
        f"I'll send that code to you here.\n"
        f"3. Settings → *Filters* → **Create a new filter**\n"
        f"4. In *Has the words* paste:\n"
        f"`{md(FORWARDING_FILTER_QUERY)}`\n"
        f"5. Choose **Forward it to** your address above.\n\n"
        f"**To add your past applications:** in Gmail search for job emails, "
        f"select them, then *⋮ → Forward as attachment* to the same address. "
        f"You can send up to 50 at a time.\n\n"
